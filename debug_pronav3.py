...
tx, ty, tz = 1000.0, 0.0, 0.0
tvx, tvy, tvz = 0.0, 100.0, 0.0
target_state = {'pos': [tx, ty, tz], 'vel': [tvx, tvy, tvz]}

pronav = ProportionalNavigation(nav_constant=5.0, max_g=50.0, activation_time_s=0.2)

solver_guided = Solver6DoF(proj, aero, env_atm, env_earth, guidance=pronav, target_state=target_state)
sol = solver_guided.solve((0, 3), [0, 0, 0], 800.0, np.deg2rad(5.0), 0.0, 100.0, max_step=0.05)
